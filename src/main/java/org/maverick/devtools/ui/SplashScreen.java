...
public class SplashScreen {
    ...
    public Element render() {
        Element[] bannerElements = Arrays.stream(BANNER_LINES)
                .map(line -> text(line).fg(AppColors.MAVERICK_GREEN).bold().length(1))
                .toArray(Element[]::new);

        return column(
                spacer(),
                column(bannerElements),
                text(buildSubtitle()).fg(AppColors.DIM_GREEN).length(1),
                spacer(),
                gauge(progress).fg(AppColors.MAVERICK_GREEN).label(statusMessage),
                spacer()).id("splash");
    }
}
